...
from environment_client import EnvironmentClient as Environment
from Environment import Environment as EnvReward
from user_client import UserClient as User
...
while True:
	...
	if is_warnable(current_state, next_action, last_action, warned_last_step):
		...
		warned_last_step = True
                ...
                r = envR.get_reward(current_state, next_action)

		current_state = env.execute_action(next_action)
                if next_action.startswith("put"):
                    env.warn('You achieved ' + str(r) + ' reward.')
		warned_last_step = False
